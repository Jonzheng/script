...
with open(r'D:\git_me\script\python\charm\data\list.json', 'r', encoding='utf-8') as inf:
  ...
  for arr in list:
    ...
    time.sleep(2)
  with open(r'D:\git_me\script\python\charm\data\up_charm0.sql', 'w', encoding='utf-8') as out:
    for chm in charmArr:
      ...
      if not name in names:
        ...
        print('chm', chm)
        if craft:
          sql = "update t_charm set craft= '{}', `unlock`='{}' where name = '{}'".format(craft, unlock, name)
          out.write(sql+';\n')
  #   for line in sqls:
  #     out.write(line+';\n')
