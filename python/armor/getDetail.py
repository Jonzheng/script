  ...
  for cd in cards:
    ...
    if 'Defense' in tn:
      ...
      for tr in trs:
        ...
        if '【' not in name:
          [icon, defence, fire, water, thunder, ice, dragon, slots, skill] = detail[name]
          suits = '|'.join(lvArr)
          icon = int(icon) + 1
          sql = "insert into t_armor values (null, '{}', '{}', '{}', '{}', {}, {}, '{}', '{}', {}, {}, {}, {}, {})".format(name, slots, skill, suits, rarity,icon, craft, defence, fire, water, thunder, ice, dragon )
          sqls.append(sql)
        elif detail[name]:
...
with open(r'D:\git_me\script\python\armor\data\list'+rarity+'.json', 'r', encoding='utf-8') as inf:
  ...
  for arr in list:
    ...
    chm = getItem(url)
    time.sleep(2)
  with open(r'D:\git_me\script\python\armor\data\t_armor'+rarity+'.sql', 'w', encoding='utf-8') as out:
    for sql in sqls:
      out.write(sql+';\n')
